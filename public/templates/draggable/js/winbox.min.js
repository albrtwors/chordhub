 ...
 self.max &&*/
    ...
    function P(a, b) {
        ...
        T &&
            ...
            t(b, "height", T + "px"),
            t(b, "line-height", T + "px"),
            t(this.body, "top", T + "px"));
        c && this.setTitle(c);
        v && this.setIcon(v);
        L
            ? this.mount(L)
            : Q
            ? (this.body.innerHTML = Q)
            : g && this.setUrl(g, Aa);
        M = M ? U(M, K) : 0;
        R = R ? U(R, K) : 0;
        N = N ? U(N, J) : 0;
        ...
        C = C ? U(C, z) : this.h;
        ca
            ? ((n || A).appendChild(this.body),
              (q = Math.max(Math.min(this.body.clientWidth + 2 * O + 1, y), w)),
              (r = Math.max(
                  Math.min(this.body.clientHeight + this.h + O + 1, z),
                  C
              )),
              this.g.appendChild(this.body))
            : ((q = q ? U(q, y) : Math.max(y / 2, w) | 0),
              (r = r ? U(r, z) : Math.max(z / 2, C) | 0));
        W = W ? U(W, c, q) : N;
        X = X ? U(X, v, r) : M;
        this.x = W;
        this.y = X;
    ...
    function ia() {
        A = document.body;
        A[(F = "requestFullscreen")] ||
            A[(F = "msRequestFullscreen")] ||
            A[(F = "webkitRequestFullscreen")] ||
            A[(F = "mozRequestFullscreen")] ||
            (F = "");
        fa =
            F &&
            F.replace("request", "exit")
    ...
    function na() {
        ...
        for (var n, p; f < a; f++)
            (d = u[f]),
                ...
                d
                    .resize((p + 1) | 0, d.h, !0)
                    .move((d.left + b[n] * p) | 0, K - d.bottom - d.h, !0),
                b[n]++;
    }
    function V(a, b) {
        function c(g) {
            ...
            a.min ||
                (A.classList.add("wb-lock"),
                (p = g.touches) && (p = p[0])
                    ? ((g = p),
                      h(window, "touchmove", f, l),
                      h(window, "touchend", d, l))
                    : (h(window, "mousemove", f, l),
                      h(window, "mouseup", d, l)),
                (v = g.pageX),
                (L = g.pageY));
        }
        function f(g) {
            ...
            H &&
                ((a.width = Math.max(
                    Math.min(a.width, a.m, J - a.x - a.right),
                    a.s
                )),
                (H = a.width !== C));
            I &&
                ((a.height = Math.max(
                    Math.min(a.height, a.l, K - a.y - a.bottom),
                    a.o
                )),
                (I = a.height !== y));
            (H || I) && a.resize();
                ...
                (a.max &&
                    (a.x =
                        (q < J / 3
                            ...
                            : J / 2 - a.width / 2) + r),
                (a.x = Math.max(
                    Math.min(a.x, a.j ? J - 30 : J - a.width - a.right),
            ...
            D &&
                (a.max && (a.y = a.top + w),
                (a.y = Math.max(
                    Math.min(a.y, a.j ? K - a.h : K - a.height - a.bottom),
                    a.top
                )),
                (D = a.y !== ca));
        ...
        function d(g) {
            ...
            p
                ? (k("touchmove", f), k("touchend", d))
                : (k("mousemove", f), k("mouseup", d));
        }
    ...
    e.restore = function () {
        E && oa();
        this.min &&
            (Z(this), this.resize().move(), this.onrestore && this.onrestore());
        this.max &&
            ((this.max = !1),
    ...
    e.maximize = function (a) {
        ...
        this.max ||
            (this.addClass("max")
                .resize(
                    J - this.left - this.right,
                    K - this.top - this.bottom,
                    !0
                )
    ...
    e.move = function (a, b, c) {
        a || 0 === a
            ? c ||
              ((this.x = a
                  ? (a = U(a, J - this.left - this.right, this.width))
                  : 0),
              (this.y = b
                  ? (b = U(b, K - this.top - this.bottom, this.height))
                  : 0))
            : ((a = this.x), (b = this.y));
        t(this.g, "left", a + "px");
        t(this.g, "top", b + "px");
    ...
    e.resize = function (a, b, c) {
        a || 0 === a
            ? c ||
              ((this.width = a ? (a = U(a, this.m)) : 0),
              (this.height = b ? (b = U(b, this.l)) : 0),
              (a = Math.max(a, this.s)),
              (b = Math.max(b, this.o)))
